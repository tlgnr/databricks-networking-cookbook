...
locals {
  ...
  aws_rds_subnet_groups                          = yamldecode(templatefile("../../configs/cross-cloud/aws/subnet-groups.yaml", { environment = var.environment, region = var.region }))
  ...
  aws_security_group_rules                       = yamldecode(templatefile("../../configs/cross-cloud/aws/security-group-rules.yaml", { environment = var.environment, region = var.region }))
}
